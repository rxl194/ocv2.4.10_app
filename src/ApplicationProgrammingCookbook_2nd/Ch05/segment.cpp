...
{
	// Read input image
	cv::Mat image= cv::imread(IMAGE_FOLDER "/group.jpg");
	if (!image.data)
		return 0; 
	...
	// Get the binary map
	cv::Mat binary;
	binary= cv::imread(IMAGE_FOLDER "/binary.bmp",0);
    ...
    // Open another image
	image= cv::imread(IMAGE_FOLDER "/tower.jpg");
	// resize for book printing
	...
	// Open another image
	image= cv::imread(IMAGE_FOLDER "/group.jpg");
	// resize for book printing
	cv::resize(image, image, cv::Size(), 0.7, 0.7);
