...
int main()
{
	...
	cv::Mat image= cv::imread(IMAGE_FOLDER "/binaryGroup.bmp",0);
	if (!image.data)
		return 0; 
	...
	cv::Mat original= cv::imread(IMAGE_FOLDER "/group.jpg");
	...
	image= cv::imread(IMAGE_FOLDER "/binaryGroup.bmp",0);
...
//	cv::convexityDefects(contours[3], hull, defects);
	...
	// get a MSER image
	cv::Mat components;
	components= cv::imread(IMAGE_FOLDER "/marais.jpg",0);
